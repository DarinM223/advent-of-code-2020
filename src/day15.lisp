...
(defun nth-spoken (input num-turns)
  (do ((turn 1)
    ...
    (cond ((<= turn (length input))
           (let ((num (aref input (- turn 1))))
             (declare (type fixnum num))
             (insert-number num turn spoken-map turns-spoken-map)
             (setf last-number-spoken num)))
          ((not (cadr (gethash last-number-spoken turns-spoken-map)))
           (insert-number 0 turn spoken-map turns-spoken-map)
           (setf last-number-spoken 0))
          (t (let* ((l (gethash last-number-spoken turns-spoken-map))
                    (num (- (the fixnum (car l)) (the fixnum (cadr l)))))
               (declare (type fixnum num))
               (insert-number num turn spoken-map turns-spoken-map)
               (setf last-number-spoken num))))
    (setf turn (+ turn 1))))
